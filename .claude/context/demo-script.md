...
4. Prerequisite (officer): complete the PROPOSAL task, upload the budget-provision document and complete BUDGET_PROVISION, then open the ADMIN_APPROVAL case and **Submit for review** (pre-stage a second project through this before recording). Then switch to **SE** → Approvals inbox → open case → approve (try Return without reason ⇒ validation). Cascade: `DESIGN_ESTIMATE` becomes ELIGIBLE, task appears for engineer; notification bell increments for engineer.
